...
It looks like, from all three analysis tools, that the percent of people unemployed and the percent of population that's white  have the largest effects on voter turnouts. Unemployment has the strongest relationship, a negative linear one. And the percent of population that's white seems to have a positive linear relationship.
...
The p-vales for income, median_age and pct_white are all statistically significant, but the rest of the variables are not. The multiple r-squared is 0.8539, which means that just over 85% of the data is explained by all of these variables. Then for the residuals, they're centered around 0, which tells us that there isn't too much variance and that the model is useful as well. All of this tells us that this model is generally useful, excpet there are specific variables that may not be useful to look at or analyze. 
...
I made my three variables the ones that showed statistical significant in the previous model. By looking at these variables, we see that model 4 (which is pct_white) actually has the strongest adjusted r-squred which is pretty interesting because in the previous model, it didn't have the strongest relationship.
...
We can use this analysis to figure out what demographic factors affect turnout the most, we can also use them to better understand which groups have higher turnout. 
