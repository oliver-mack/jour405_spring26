...
wh_paid |> summarize(mean_wh_paid = mean(SALARY), median_wh_paid = median(SALARY), sd_wh_paid = sd(SALARY))
...
This distribution is skewed to the right. Most salaries cluster below around $125,000, with many of the data points sitting around 65/70k.
...
It follows the empirical rule somewhat closely. This tells us that the distribution is not normal. Since there are less data points within the first and second SD, this tells us that the data is not clustered directly around the mean and is likely more spread out. 100% of the data being within 3 SD tells us that there are no major outliers. 
...
If this data was normally distributed, we would expect to see 16.88% of White House salaries be above $150,000. In reality, 18.18% of White House salaries are above $150,000.
...
Since the middle range here is smaller in reality than in the expected in a normal distribution, we can infer that the White House salaries are rather polarized compared to a normal distribution, and there are likely a lot of people who make either less than 80k or more than 120k.
...
The biggest difference is in the 60-90k region. This is because many more workers have salaries in this range than the expected count would be. The observed counts here also point to a left-skewed distribution, rather than a normal distribution.
...
A dive into the White House's wage gap.

Less than one-third of White House workers make between $80,000 and $120,000 dollars. Instead, many make less and some make a lot more, creating two large salary groups on 1600 Pennsylvania Ave. 

I would compare these probabilities to those of other workplaces, specifically to federal workers/other federal departments. It seems like the White House (somewhat) has two clear tiers of workers/salaries. I think it would be interesting to analyze if this is replicated across other parts of the federal government or not.
